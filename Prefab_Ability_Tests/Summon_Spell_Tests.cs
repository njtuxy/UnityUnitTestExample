...
namespace Ability_Prefab_Tests
{
	...
	[TestFixture]
	public class Summon_Spell_Tests {


		static object[] prefabs = 
		{
			new object[] {"Spell_SummonCompanion.prefab"},
			new object[] {"Spell_SummonCreature.prefab"},
			new object[] {"Spell_SummonCreature_DPS.prefab"},
			new object[] {"Spell_SummonZodiac_GoldDragon.prefab"},
			new object[] {"Spell_SummonZodiac_Placeholder.prefab"}
		...
		[Test, TestCaseSource("prefabs")]
		public void Has_A_Spawn_Component (string prefab_name)
		{
			Spell_Test_Helper myTestObject = new Spell_Test_Helper(prefab_name);
			Assert.IsTrue(myTestObject.spell_spawn_component !=null, testFailureMsg(myTestObject.name));
		}
